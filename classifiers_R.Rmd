...
xgb <- xgboost(data = train_features, 
              label = train_label,
...
nrounds = c(10,100), # no. of passes of the data, i.e. no. of iterations
...
Jan 19: optimal grid: nround = 100, max_depth = 5, eta = 0.1, gamma = 1, colsample_bytree = 0.7, min_child_weight = 5, subsample = 0.8

```{r}
# fitting the optimal xgb model
...
xgb_importance <- xgb.importance(model = xgb, label = train_label)
...
preds_proba_xgb <- predict(xgb_opt, test_features)[0:length(y_test)] 
